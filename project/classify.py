...
from ..classifiers import BaseClassifier, load_classifier, get_thresholds_from_file
from ..featurizers import load_featurized, save_featurized
from ..utils import load_dictionary_from_file, get_settings, uri_open
...
def main():
    ...
    parser.add_argument('-c', '--classifier-info', type=FileType('rb'), metavar='<classifier_file>', help='Display information about classifier.')
    ...
    fit_parser.add_argument('classifier_type', type=str, metavar='<classifier_type>', nargs='?', default=None, help='Name of classifier model to fit.')
    fit_parser.add_argument('-f', '--featurized', type=FileType('rb'), metavar='<featurized>', help='Fit model on featurized instances.')
    fit_parser.add_argument('-o', '--output', type=FileType('wb'), metavar='<classifier_file>', required=True, help='Save trained classifier model here.')

    evaluate_parser = subparsers.add_parser('evaluate', help='Evaluate a classifier.')
    evaluate_parser.add_argument('classifier_file', type=FileType('rb'), metavar='<classifier_file>', help='Model file to use for evaluation.')
    evaluate_parser.add_argument('featurized_file', type=FileType('rb'), metavar='<featurized_file>', help='Evaluate model on featurized instances.')
    evaluate_parser.add_argument('-t', '--thresholds', type=FileType('r'), metavar='<thresholds>', required=False, help='Threshold file to use for prediction.')
    evaluate_parser.add_argument('-p', '--save-probabilities', type=FileType('wb'), metavar='<probabilities_file>', required=False, help='Save evaluation probabilities; useful for calibration.')

    predict_parser = subparsers.add_parser('predict', help='Predict using a classifier.')
    predict_parser.add_argument('classifier_file', type=FileType('rb'), metavar='<classifier_file>', help='Model file to use for prediction.')
    predict_parser.add_argument('featurized_file', type=FileType('rb'), metavar='<featurized_file>', help='Predict labels of featurized instances.')
    predict_parser.add_argument('-t', '--thresholds', type=FileType('r'), metavar='<thresholds>', required=False, help='Threshold file to use for prediction.')
    predict_parser.add_argument('-o', '--output', type=FileType('wb'), metavar='<prediction_file>', help='Save results of prediction here.')
    ...
    evaluate_parser = subparsers.add_parser('info', help='Display information regarding classifier.')
    evaluate_parser.add_argument('classifier_file', type=FileType('rb'), metavar='<classifier_file>', help='Model file to display information onabout.')
    ...
    if A.mode == 'fit':
        ...
        if output_format in ['csv', 'tsv', 'txt', '']:
            if A.probs:
                writer = csv.DictWriter(TextIOWrapper(A.output, encoding='utf-8', newline='\n'), fieldnames=['ID'] + list(classifier.classes_) + ['p({})'.format(c) for c in classifier.classes_], dialect='excel' if output_format == '.csv' else 'excel-tab')
            else:
                writer = csv.DictWriter(TextIOWrapper(A.output, encoding='utf-8', newline='\n'), fieldnames=['ID'] + list(classifier.classes_), dialect='excel' if output_format == '.csv' else 'excel-tab')
        ...
        logger.info('Writing predictions to <{}> in {} format...'.format(A.output.name, output_format.upper()))

        classifier = load_classifier(A.classifier_file)
        thresholds = 0.5

        featurizer_uuid, X_featurized, X_meta, Y_labels, featurized_at = load_featurized(A.featurized_file)
        Y_proba, Y_predict_binarized = classifier.predict_and_proba(X_featurized, thresholds)
